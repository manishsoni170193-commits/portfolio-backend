...
def _get_yahoo_crumb(force_refresh=False):
    if _yahoo_session["crumb"] and not force_refresh:
    ...
    for attempt in range(3):
        try:
            ...
            # Yahoo returns plain text errors (not JSON) when rate-limiting this endpoint
            if not crumb or "Too Many Requests" in crumb or "error" in crumb.lower() or len(crumb) > 50:
                last_error = f"Yahoo rate-limited the auth request (attempt {attempt+1}/3)"
    ...
    raise Exception(f"Could not get Yahoo auth token after 3 attempts: {last_error}")
...
def get_analyst(symbol):
    ...
    try:
        ...
        if resp.status_code == 401:
            ...
            cookies, crumb = _get_yahoo_crumb(force_refresh=True)
            params["crumb"] = crumb
            resp = requests.get(url, headers=HEADERS, params=params, cookies=cookies, timeout=10)
        resp.raise_for_status()
        data = resp.json()
